...
def train_step(images):
    ...
    # noise = seed
    with tf.GradientTape() as gen_tape, tf.GradientTape() as disc_fe_tape, tf.GradientTape() as disc_final_tape:
        generated_images = generator(noise, training=True)

        real_fx = discriminator_fe(images, training=True)
        fake_fx = discriminator_fe(generated_images, training=True)

        real_output = discriminator_final(real_fx, training=True)
        fake_output = discriminator_final(fake_fx, training=True)

        gen_loss = generator_loss(fake_output) + feature_matching(fake_fx, real_fx)
        disc_loss = discriminator_loss(real_output, fake_output)

    gradients_of_generator = gen_tape.gradient(gen_loss, generator.trainable_variables)
    gradients_of_discriminator_fe = disc_fe_tape.gradient(disc_loss, discriminator_fe.trainable_variables)
    gradients_of_discriminator_final = disc_final_tape.gradient(disc_loss, discriminator_final.trainable_variables)

    generator_optimizer.apply_gradients(zip(gradients_of_generator, generator.trainable_variables))
    discriminator_fe_optimizer.apply_gradients(zip(gradients_of_discriminator_fe, discriminator_fe.trainable_variables))
    discriminator_final_optimizer.apply_gradients(
        zip(gradients_of_discriminator_final, discriminator_final.trainable_variables))
...
generator_optimizer = tf.keras.optimizers.Adam(learning_rate=1e-4, beta_1=0.5, beta_2=0.999, epsilon=1e-7)
discriminator_fe_optimizer = tf.keras.optimizers.Adam(learning_rate=2e-4, beta_1=0.5, beta_2=0.999, epsilon=1e-7)
discriminator_final_optimizer = tf.keras.optimizers.Adam(learning_rate=2e-4, beta_1=0.5, beta_2=0.999, epsilon=1e-7)

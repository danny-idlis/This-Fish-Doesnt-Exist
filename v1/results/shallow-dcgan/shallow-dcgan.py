...
def make_generator_model():
    ...
    model.add(layers.Dense(80*35*128, use_bias=False, input_shape=(noise_dim,)))
    model.add(layers.BatchNormalization()) # Normalize and scale inputs or activations. See remark bellow
    model.add(layers.LeakyReLU())
    model.add(layers.Reshape((35, 80, 128)))

    model.add(layers.Conv2DTranspose(64, 5, strides=2, padding='same', use_bias=False))
    model.add(layers.BatchNormalization())
    model.add(layers.LeakyReLU())

    model.add(layers.Conv2DTranspose(32, 3, strides=1, padding='same', use_bias=False))
    model.add(layers.BatchNormalization())
    model.add(layers.LeakyReLU())

    model.add(layers.Conv2DTranspose(16, 1, strides=2, padding='same', use_bias=False))
    model.add(layers.BatchNormalization())
    model.add(layers.LeakyReLU())
    ...
    model.add(layers.Conv2DTranspose(3, 3, strides=1, padding='same', use_bias=False, activation='tanh'))
...
def make_discriminator_model():
    ...
    model.add(layers.Conv2D(8, 3, strides=1, padding='same', input_shape=[140, 320, 3]))
    model.add(layers.BatchNormalization())
    model.add(layers.LeakyReLU())

    model.add(layers.Conv2D(16, 1, strides=2, padding='same'))
    model.add(layers.BatchNormalization())
    model.add(layers.LeakyReLU())
